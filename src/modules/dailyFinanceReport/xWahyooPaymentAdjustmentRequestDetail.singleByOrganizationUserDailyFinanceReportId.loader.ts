...
@Injectable()
export class XWahyooPaymentAdjustmentRequestDetailSingleByOrganizationUserDailyFinanceReportIdLoader
  ...
  > {
    return new DataLoader<string, XWahyooPaymentAdjustmentRequestDetail>(
      async keys => {
        const xWahyooPaymentAdjustmentRequestDetailModels: XWahyooPaymentAdjustmentRequestDetailModel[] = await this.repository.findByOrganizationUserDailyFinanceReportIds(
          keys as string[]
        );
        ...
        return keys.map(key =>
          xWahyooPaymentAdjustmentRequestDetailList.find(
            xWahyooPaymentAdjustmentRequestDetail =>
              xWahyooPaymentAdjustmentRequestDetail.organizationUserDailyFinanceReportId ===
              key
          )
        );
      }
