...
@Injectable()
export class ReconcileHubsterOrderUnprocessedUseCase implements IUseCase {
  constructor(
  ...
  async checkAndReconcileHubsterOrderByStoreId({
    ...
    try {
      // default values: limit = 7000, period = from 7 days ago until yesterday
      let limit = 7000;
      let startTimeTodayMoment = moment
        .tz(TimeZone.ASIA_JAKARTA)
        .subtract(7, 'days')
        .startOf('day');
      let endTimeTodayMoment = moment
        .tz(TimeZone.ASIA_JAKARTA)
        .subtract(1, 'days')
        .endOf('day');
      ...
      if (dto.start) {
        if (dto.start.indexOf('days') > -1) {
          ...
          startTimeTodayMoment = moment
            ...
            .startOf('day');
        } else if (dto.start.indexOf('weeks') > -1) {
          // e.g. dto.start = 'weeks-1'
          const arrStr = dto.start.split('-');
          console.log('subtract weeks:', Number(arrStr[1]));
          startTimeTodayMoment = moment
            .tz(TimeZone.ASIA_JAKARTA)
            .subtract(Number(arrStr[1]), 'weeks')
            .startOf('day');
        }
      }
        ...
        if (id) {
          ...
          const payloadEvent = {
            ...
            metadata: {
              ...
              resourceId: 'manual reconcile (unprocessed)',
