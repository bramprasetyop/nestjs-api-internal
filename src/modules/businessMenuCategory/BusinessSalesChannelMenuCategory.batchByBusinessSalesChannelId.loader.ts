...
@Injectable()
export class BusinessSalesChannelMenuCategoryBatchByBusinessSalesChannelIdLoader
  ...
  generateDataLoader(): DataLoader<string, BusinessSalesChannelMenuCategory[]> {
    return new DataLoader<string, BusinessSalesChannelMenuCategory[]>(
      async keys => {
        const businessSalesChannelMenuCategories: BusinessSalesChannelMenuCategoryModel[] = await this.repository.findByBusinessSalesChannelIds(
          keys as string[]
        );
        ...
        return keys.map(key =>
          businessMenuCategoryList.filter(
            businessSalesChannelMenuCategory =>
              businessSalesChannelMenuCategory.businessSalesChannelId === key
          )
        );
      }
    );
