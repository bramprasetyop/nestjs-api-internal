...
@Injectable()
export class BusinessSalesChannelMenuItemBatchByMenuItemIdLoader
  ...
  generateDataLoader(): DataLoader<string, BusinessSalesChannelMenuItem[]> {
    return new DataLoader<string, BusinessSalesChannelMenuItem[]>(
      async keys => {
        const businessSalesChannelMenuItems: BusinessSalesChannelMenuItemModel[] = await this.repository.findByMenuItemIds(
          keys as string[]
        );
        ...
        return keys.map(key =>
          businessMenuItemList.filter(
            businessSalesChannelMenuItem =>
              businessSalesChannelMenuItem.businessMenuItemId === key
          )
        );
      }
    );
