...
@Injectable()
export class BusinessOutletLeadTrainingUseCase implements IUseCase {
  constructor(
  ...
  async execute(
    dto: BusinessOutletLeadTrainingRequest
  ): Promise<BusinessOutletLead> {
    const { businessOutletLeadId, trainingSchedule } = dto;
    ...
    if (
      !(
        businessOutletLead.ongoingStatus ===
          BusinessOutletLeadOngoingStatus.term_signed ||
        businessOutletLead.ongoingStatus ===
          BusinessOutletLeadOngoingStatus.training_scheduled
      )
    ) {
      throw new Error(
        'Status before scheduling training must be "term_signed" or "training_scheduled"'
      );
    }

    let data = {
      status: BusinessOutletLeadStatus.ongoing,
      ongoingStatus: BusinessOutletLeadOngoingStatus.training_scheduled,
      trainingSchedule,
